...
export default function AwardSheet({ student, balance, onConfirm, onClose }) {
    ...
    <div className="action-overlay" onClick={onClose}>
      <div className="action-sheet" onClick={e => e.stopPropagation()}>
        ...
        <header className="action-sheet-header">
          <h2 className="action-sheet-title">🏆 Award — {student}</h2>
        ...
        <div className="action-sheet-body">
          ...
          <p className="action-balance">Balance: {balance} pts</p>

          <textarea className="action-input" rows={2} value={note}
            onChange={e => setNote(e.target.value)}
            placeholder="What awesome thing did they do?" />
        </div>

        <div className="action-sheet-footer">
          <button className="action-cancel-btn" onClick={onClose}>Cancel</button>
          <button className="action-confirm-btn action-confirm-btn--award" onClick={handleConfirm}>
            Award {value} pts! 🎉
          </button>
        </div>
      </div>
